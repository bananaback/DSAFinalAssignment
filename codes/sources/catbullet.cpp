...
CatBullet::CatBullet(Game& game, float x, float y, float width, float height, float angle) : PlayerBullet(game, x, y, width, height, angle) {
	...
	_damageDeal = 15;
	_angle = angle;
	_animation = std::make_shared<Animation>(game.ra_ptr->_imageResources[game.ra_ptr->IMAGE::CAT], 0, 0, getFrameWidth(), getFrameHeight(), 1, 1, "cat");
	_durability = 2;
...
void CatBullet::draw(Game& game) {
	_animation->draw(game, _x + _width / 2, _y + _height / 2, _displayAngle, 2, 2*_scaleY, 1.f * getFrameWidth() / 2, 1.f * getFrameHeight() / 2);
