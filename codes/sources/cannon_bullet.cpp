...
CannonBullet::CannonBullet(Game& game, float x, float y, float width, float height, float angle) : PlayerBullet(game, x, y, width, height, angle) {
	...
	_damageDeal = 20;
	_angle = angle;
	_animation = std::make_shared<Animation>(game.ra_ptr->_imageResources[IMAGE::CANNONBALL], 0, 0, getFrameWidth(), getFrameHeight(), 1, 1, "cannonball");
	_durability = 1;
...
void CannonBullet::draw(Game& game) {
	_animation->draw(game, _x + _width / 2, _y + _height / 2, 0, 2, 2, getFrameWidth() / 2.f, getFrameHeight() / 2.f);
